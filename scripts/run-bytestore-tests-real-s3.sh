...
#
# Required env:
#   ATTESTA_REAL_S3_BUCKET   the bucket name (REQUIRED)
...
# s3:ListBucket on $ATTESTA_REAL_S3_BUCKET.
#
# Usage:
...
# ── Validate inputs ───────────────────────────────────────────────
if [ -z "${ATTESTA_REAL_S3_BUCKET:-}" ]; then
    echo "FATAL: ATTESTA_REAL_S3_BUCKET not set"
    echo
    echo "  export ATTESTA_REAL_S3_BUCKET=<your-bucket-name>"
    echo "  $0"
    exit 1
fi
...
echo "   bucket: ${ATTESTA_REAL_S3_BUCKET}"
echo "   region: ${AWS_REGION:-${AWS_DEFAULT_REGION}}"
echo

# Real-S3 mode signal: ATTESTA_TEST_S3_REAL=1.
# requireS3 detects this and uses default credential chain + virtual-host URLs.
unset ATTESTA_TEST_S3_ENDPOINT
unset ATTESTA_TEST_S3_PATH_STYLE
export ATTESTA_TEST_S3_REAL=1
export ATTESTA_TEST_S3_BUCKET="${ATTESTA_REAL_S3_BUCKET}"
export ATTESTA_TEST_S3_REGION="${AWS_REGION:-${AWS_DEFAULT_REGION}}"
...
echo
echo "  aws s3 ls s3://${ATTESTA_REAL_S3_BUCKET}/test/ --recursive || echo '(no leftovers — good)'"
