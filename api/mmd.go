...
GET /v1/admission/mmd — the operator's published Maximum Merge Delay.
This is the SLA on the Sequencer's drain latency: the operator
asserts that any entry it issues an SCT for will be merged into the
log within MMD seconds.

RFC 6962-aligned. Consumers verify the operator's promised
...
// NewMMDHandler returns a handler that publishes the operator's
